...
export function TemplatePreviewSheet({
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end bg-black/40">
      <div className="max-h-[85%] overflow-y-auto rounded-t-3xl bg-surface pb-safe">
        <div className="sticky top-0 flex items-start justify-between gap-3 border-b border-line bg-surface px-5 py-3.5">
          <div className="min-w-0">
            ...
            <p className="text-[12.5px] text-ink-muted">
              Template · {preview?.exercises.length ?? 0} exercises · {preview?.totalSets ?? 0}{' '}
              planned sets
            </p>
          </div>
          <button
        ...
        <div className="sticky bottom-0 mt-2 flex gap-2 border-t border-line bg-surface px-4 py-3">
          {onEdit && (
            <Button variant="secondary" size="lg" onClick={onEdit} aria-label="Edit template">
              <Pencil size={17} />
            </Button>
          )}
          <Button
