// Drag to reorder, drag-onto to group. Two drop outcomes: a gap between cards
// reorders; a card's middle band groups the two (the "group" gesture is opt-in via
// onGroup; the badge text is whatever the caller passes as groupHint).
...
function intentTarget(intent: DropIntent): string | number | null {
  if (intent.kind === 'group') return intent.targetId
...
export function DragList({
  ...
  // When omitted, there's no group gesture and the middle band reorders like the rest.
  onGroup?: (draggedId: string, targetId: string) => void
  children: ReactNode
}) {
  ...
  // The lifted card follows the finger by writing `transform` straight to the node,
  // not through state: a setState per pointermove re-renders every card. Only the
  // drop intent uses state.
  ...
  const resolveIntent = useCallback(
    (draggedId: string, clientY: number): DropIntent => {
      ...
      for (const [index, box] of boxes.entries()) {
        ...
        // The middle band groups (when enabled); the outer thirds reorder around
        // the card. With no `onGroup`, the whole card reorders.
        const offset = (clientY - top) / height
        const bandStart = (1 - GROUP_BAND) / 2
        if (onGroup && offset > bandStart && offset < 1 - bandStart) {
          return { kind: 'group', targetId: box.id }
        }
  ...
  const finish = useCallback(
    (draggedId: string, intent: DropIntent) => {
      if (intent.kind === 'group') {
        onGroup?.(draggedId, intent.targetId)
...
export function DragItem({
  ...
  id: string
  index: number
  /** Badge shown when this card is the group (drag-onto) target, e.g. "Superset with X". */
  groupHint?: ReactNode
  children: ReactNode
}) {
  ...
  const isGroupTarget = intent.kind === 'group' && intent.targetId === id
